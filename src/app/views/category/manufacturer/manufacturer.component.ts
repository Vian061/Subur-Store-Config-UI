...
export class ManufacturerComponent {
  ...
  loadData() {
    ...
    this.checkAll = false;
    this.selectedData = [];
    this.networkService.get(Constants.UrlEndpoint.manufacturerEndpoint + "/POSData").subscribe({
      next: (response) => {
  ...
  submit() {
    ...
    this.networkService.post(Constants.UrlEndpoint.manufacturerEndpoint, data).subscribe({
      next: (response) => {
        this.messageService.add({
          severity: "success",
          summary: "Success",
